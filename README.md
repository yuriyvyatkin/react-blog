...
1. Список постов (главная страница)
   ...
   1.8. При загрузке данных с сервера отображается сперва спиннер предзагрузки, а только потом подгруженный контент (создана дополнительная искусственная задержка в 0.5 секунд, чтобы спиннер было видно).
